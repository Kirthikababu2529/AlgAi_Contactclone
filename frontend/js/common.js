...
// Example usage to generate a random code of length 8
const randomCode = generateRandomCode(8);
console.log(randomCode);
  // Output: "31662HugeTowel"
...
function joinRoom() {
    ...
    let roomName = roomNameElement ? roomNameElement.value.trim() : '';
    if (roomName) {
        roomName = filterXSS(roomName); 
        const video = 'https://etutor.algaitech.com/join/' + roomName;
        window.location.href =video; 
        // legacy code
        // window.location.href = '/join/' + roomName;
        console.log('Joining room:', roomName);
        const clmsPageUrl = 'https://etutor.algaitech.com/public/js/clms.html?room=' + roomName;
        console.log('Opening clms.html:', clmsPageUrl);
        const newWindow = window.open(clmsPageUrl, '_blank');
        if (newWindow) {
            video.opener.focus(); 
        } else {
            alert('You will be redialed');
        }
